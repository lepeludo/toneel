...
<body>
	...
	<h1>${toneelstuk.naamstuk} door ${toneelstuk.naamauteur}</h1>
	<c:url value="/delete" var="verwijderURL">
			<c:param name="id" value="${toneelstuk.id}" />
		</c:url>
		<sf:form action="${verwijderURL}" method="post">
			<input type="submit" value="Verwijderen" />
		</sf:form>
	<c:url value="/doupdate" var="url" />
	<div class="wit">
		<sf:form action='${url}' method="post" commandName="toneelstuk">
			...
			<sf:hidden path="id" />
			<button type="submit" class="btn btn-primary">Update stuk</button>
			<br />
			<br />
		</sf:form>
	</div>
